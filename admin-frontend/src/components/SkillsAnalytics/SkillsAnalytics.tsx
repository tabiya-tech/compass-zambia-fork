...
import { Box, Paper, Typography, useTheme, Select, MenuItem, LinearProgress } from "@mui/material";
import { useTranslation } from "react-i18next";
import { mockSkillsDemandSupply, mockSkillsGapSector } from "src/data/mockSkillsAnalytics";

const SkillsAnalytics: React.FC = () => {
  const theme = useTheme();
  const { t } = useTranslation();

  const supplyData = [...mockSkillsDemandSupply]
    .filter((s) => s.supplyPct > 0)
    .sort((a, b) => b.supplyPct - a.supplyPct)
    .slice(0, 10);
  const demandData = [...mockSkillsDemandSupply]
    .filter((s) => s.demandPct > 0)
